...
# Expects empty folders named SimAngNPY, SimIDNPY, SimPosNPY
...
center_id = [i for i, x in enumerate(traj[0].particles.types) if x=="Square"]

for f in range(num_frames):
    snap = traj[f]
    center_positions = np.array([pos for idx, pos in enumerate(snap.particles.position) if snap.particles.typeid[idx] in center_id])[:, :2]
    orientations = np.array([180*quat_to_zaxis_angle(quat)/np.pi for idx, quat in enumerate(snap.particles.orientation) if snap.particles.typeid[idx] in center_id])
    num_parts = len(center_positions)

    np.save("SimPosNPY/"+str(f)+"_pos.npy", center_positions)
    np.save("SimAngNPY/"+str(f)+"_ang.npy", orientations)
